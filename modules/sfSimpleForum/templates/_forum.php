...
<tr>
  <td class="forum_name">
    <?php echo link_to($forum->getName(), '@forum_name?forum_name='.$forum->getSlug()) ?><br />
  ...
  <td class="forum_recent">
    <?php if ($forum->getLatestPostId()): ?>
      ...
      <?php echo link_to($latest_post->getTitle(), '@forum_message?id='.$latest_post->getId()) ?><br />
      <?php echo __('%date% ago by %author%', array(
        ...
        '%author%' => link_to(get_partial('sfSimpleForum/author_name', array('author' => $latest_post->getAuthorName(), 'sf_cache_key' => $latest_post->getAuthorName())), '@forum_user?username='.$latest_post->getAuthorName())
        ), 'sfSimpleForum') ?>
    <?php endif ;?>
  </td>
</tr>